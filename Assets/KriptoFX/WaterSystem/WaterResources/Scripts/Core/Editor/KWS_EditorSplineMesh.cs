...
namespace KWS
{
    internal class KWS_EditorSplineMesh
    {
        ...
        Vector3 DrawMoveHandle(Vector3 position, Color color)
        {
            var newPos = position;
            Handles.color = color;
            var fmh_192_53_638837494062791760 = Quaternion.identity; newPos = Handles.FreeMoveHandle(newPos, GetHandleSphereSize(position), Vector3.one, Handles.CylinderHandleCap);
            return newPos;
        }
